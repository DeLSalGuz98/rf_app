...
export function EditProjectData() {
  ...
  const onSubmit = async (data) => {
    const updatedFields = getUpdatedFields({...data, fecha_fin: calcularFechaFin(data.fecha_inicio, data.plazo_dias)}, dataProject);
  ...
  return (
    <Container>
      <FormComponent methods={methods} onSubmit={onSubmit} title={`Actualizar Datos - Proyecto ${dataProject.nombre_proyecto}`}>
        <Row>
        ...
        <BtnSubmitForm />
        <Button className="w-100 mt-2" variant="outline-secondary" onClick={backPage}>Regresar</Button>
      </FormComponent>
    </Container>
  );
}